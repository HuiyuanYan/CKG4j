...
hadoop jar {hadoop_streaming_path} \
    ...
    -input {input_dir} \
    -output {output_dir} \
    -mapper 'python {ckg4j_root}/src/task2/mapper.py' \
    -reducer 'python {ckg4j_root}/src/task2/reducer.py' \
...
"""
    ...
    try:
        ...
        logger.info(f"Task2 finished successfully, output_dir: {local_path}")
    except:
...
if __name__ == "__main__":
    logger.info(f"Start running task2, ckg4j_root: {ckg4j_root}, hadoop_streaming_path: {hadoop_streaming_path}")
    ...
    logger.info(f"Task2 args: {args._get_args()}")
